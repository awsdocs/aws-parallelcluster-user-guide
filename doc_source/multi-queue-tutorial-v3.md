...
## Log in to the head node<a name="multi-queue-tutorial-v3-log-into-head-node"></a>
...
Other nodes are all in the power saving state, indicated by the `~` suffix in node state, with no EC2 instances backing them\. The default queue is indicated by a `*` suffix after its queue name\. `spot` is your default job queue\.
