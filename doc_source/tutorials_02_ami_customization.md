...
1. In the AMI list, find the AMI that corresponds to the Region you are using\. The AMI list to use must match the version of AWS ParallelCluster you are using\. Run `pcluster version` to verify the version\. For AWS ParallelCluster 2\.8\.1 go to [https://github\.com/aws/aws\-parallelcluster/blob/v2\.8\.1/amis\.txt](https://github.com/aws/aws-parallelcluster/blob/v2.8.1/amis.txt)\. To select another version, use the same link, choose the **Tag: 2\.8\.1** button, select the **Tags** tab, and then select the appropriate version\.
