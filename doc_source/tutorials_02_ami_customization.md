...
1. In the AMI list, find the AMI that corresponds to the specific Region that you use\. The AMI list that you choose must match the version of AWS ParallelCluster that you use\. Run `pcluster version` to verify the version\. For AWS ParallelCluster version 2\.11\.1, go to [https://github\.com/aws/aws\-parallelcluster/blob/v2\.11\.1/amis\.txt](https://github.com/aws/aws-parallelcluster/blob/v2.11.1/amis.txt)\. To select another version, use the same link, choose the **Tag: 2\.11\.1** button, select the **Tags** tab, and then select the appropriate version\.
